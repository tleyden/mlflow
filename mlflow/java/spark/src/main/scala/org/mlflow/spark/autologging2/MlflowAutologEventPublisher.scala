 ...
 * details.
 */
private[autologging2] trait MlflowAutologEventPublisherImpl {
  private val logger = LoggerFactory.getLogger(getClass)

  private[autologging2] var sparkQueryListener: SparkListener = _
  private val executor = new ScheduledThreadPoolExecutor(1)
  private[autologging2] val subscribers =
  ...
  println(f"Custom MlflowAutologEventPublisherImpl autologging2: ${this}.  subscribers: ${subscribers}")
  ...
  private def isInReplAwareContext: Boolean = {
    ...
    println(f"isInReplAwareContext, replid: ${replId}")
    if (replId.isDefined) {
      return true
    }
  ...
  def init(gcDeadSubscribersIntervalSec: Int = 1): Unit = synchronized {
    println("Custom MlflowAutologEventPublisherImpl init autologging2")
  ...
  def register(subscriber: MlflowAutologEventSubscriber): Unit = synchronized {
    ...
    println(f"register subscriber ${subscriber} with replid ${subscriber.replId}")
  ...
  // order within tests
  private[autologging2] def getSubscribers: Seq[(String, MlflowAutologEventSubscriber)] = {
    subscribers.asScala.toSeq
  }

  /** Unregister subscribers broken e.g. due to detaching of the associated Python REPL */
  private[autologging2] def unregisterBrokenSubscribers(): Unit = {
  ...
  private[autologging2] def publishEvent(
      ...
      case SparkTableInfo(path, version, format) =>
        for ((replId, listener) <- getSubscribers) {
          if (replIdOpt.isEmpty || replId == replIdOpt.get) {
            try {
              println(f"publishEvent replIdOpt: ${replIdOpt} notifying listener ${listener} with replid ${replId}")
